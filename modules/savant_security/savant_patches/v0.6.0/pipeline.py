...
class NvDsPipeline(GstPipeline):
    ...
    def _update_meta_for_single_frame(
        ...
        # PATCH(video-analytics): stale frames of a removed source (PTS-reset
        # race) can reach output meta preparation after the registry entry was
        # released. Skip them instead of raising KeyError.
        try:
            source_info = self._sources.get_source(video_frame.source_id)
        except KeyError:
            self._logger.warning(
                'Skipping output meta for frame of source %s with IDX %s and '
                'PTS %s: source was removed (PTS-reset race). '
                '[video-analytics patch]',
                video_frame.source_id,
                frame_idx,
                frame_pts,
            )
    ...
    def _on_muxer_sink_pad_peer_eos(
        ...
        # PATCH(video-analytics): on duplicate/late EOS the source may already
        # be gone from the registry; nothing left to remove in that case.
        try:
            source_info = self._sources.get_source(source_id)
        except KeyError:
            self._logger.warning(
                'Got EOS for source %s which is not registered anymore; '
                'ignoring. [video-analytics patch]',
                source_id,
            )
            return Gst.PadProbeReturn.PASS
        try:
