...
class NvDsBufferProcessor(GstBufferProcessor):
    ...
    def _prepare_input_frame(
        ...
        # PATCH(video-analytics): a source reset (non-monotonous PTS -> EOS ->
        # branch removal) can release the SourceInfo registry entry while stale
        # buffers for that source are still queued in the muxer. Upstream code
        # raises KeyError here, which posts a bus error and stops the module.
        # Skip such zombie frames instead of crashing the whole pipeline.
        try:
            source_info = self._sources.get_source(video_frame.source_id)
        except KeyError:
            self.logger.warning(
                'Skipping input preparation for frame of source %s with IDX %s '
                'and PTS %s: source was removed (PTS-reset race). '
