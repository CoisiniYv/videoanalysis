...
class NvInferProcessor:
    ...
    def _process_custom_model_output(self, buffer: Gst.Buffer):
        ...
        for nvds_frame_meta in nvds_frame_meta_iterator(nvds_batch_meta):
            ...
            # PATCH(video-analytics): the source registry entry may already be
            # released by a PTS-reset-triggered source removal while this batch
            # is still in flight. Skip the stale frame instead of raising
            # KeyError (which posts a bus error and stops the module).
            try:
                source_info = self._sources.get_source(source_id)
            except KeyError:
                self._logger.warning(
                    'Skipping "%s" custom model output for frame of source %s '
                    'with IDX %s and PTS %s: source was removed '
                    '(PTS-reset race). [video-analytics patch]',
                    self._element_name,
                    source_id,
                    frame_idx,
