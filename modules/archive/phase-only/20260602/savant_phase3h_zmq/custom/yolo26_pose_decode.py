...
def _ensure_3d(raw: np.ndarray, layout: str) -> np.ndarray:
    if raw.ndim == 3:
        return raw
    if raw.ndim != 2:
        raise ValueError(f"Expected 2-D or 3-D tensor, got shape {raw.shape}")
    H, W = raw.shape
...
def _decode_raw_56(raw: np.ndarray, cfg: DecoderConfig) -> List[PoseDetection]:
    ...
    for b in range(raw.shape[0]):
        results.extend(_decode_raw_56_one(raw[b], cfg))
    return results
...
def _decode_raw_56_one(tensor: np.ndarray, cfg: DecoderConfig) -> List[PoseDetection]:
    ...
    if C != cfg.num_channels:
        raise ValueError(f"raw_56: expected {cfg.num_channels} channels, got {C}.")
...
def _nms(dets: List[PoseDetection], iou_threshold: float, top_k: int) -> List[int]:
    if not dets:
        return []
