...
def _ensure_3d(raw: np.ndarray, layout: str) -> np.ndarray:
    ...
    if layout == DECODER_LAYOUT_POST_NMS_57:
        # Expected: (N, 57) or (57, N).  The smaller dim is the channel.
        if W == 57:
            # Already (N, 57) — just add batch
            return raw[None, :, :]
        if H == 57:
            # (57, N) — transpose to (N, 57) then add batch
            return raw.T[None, :, :]
    else:
        # raw_56: expected (56, N) or (N, 56).  Channel dim is 56.
        if H == 56:
            # (56, N) → add batch → (1, 56, N)
            return raw[None, :, :]
        if W == 56:
            # (N, 56) → transpose to (56, N) → add batch
            return raw.T[None, :, :]

    # Fallback: if we can't determine, try treating as (N, C) with small C
    if min(H, W) <= 57:
        if H < W:
            # (C, N) with C = smaller
            return raw.T[None, :, :]
...
def _decode_raw_56_one(tensor: np.ndarray, cfg: DecoderConfig) -> List[PoseDetection]:
    """Decode a single batch item.  ``tensor`` shape = (channels, predictions)."""
    C, N = tensor.shape
...
def _decode_post_nms_57(raw: np.ndarray, cfg: DecoderConfig) -> List[PoseDetection]:
    """Decode post_nms_57 layout.

    ``raw`` shape = (batch, num_detections, 57)::

        0:4   = bbox (x1, y1, x2, y2)  — pixel coordinates relative to 640x640
        4     = confidence
        5     = class_id
        6:57  = 17 keypoints x 3 (x, y, confidence) — pixel coordinates
    """
    results: List[PoseDetection] = []
    for b in range(raw.shape[0]):
...
def _decode_post_nms_57_one(tensor: np.ndarray, cfg: DecoderConfig) -> List[PoseDetection]:
    """Decode a single batch item.  ``tensor`` shape = (N, 57)."""
    N, C = tensor.shape
    if C != 57:
    ...
    # bbox (x1, y1, x2, y2) — already in pixel coords relative to model input
    x1 = tensor[:, 0]
    y1 = tensor[:, 1]
    x2 = tensor[:, 2]
    y2 = tensor[:, 3]

    # keypoints  (cols 6:57)  ->  (M, 17, 3)
    kpts_flat = tensor[:, 6:57]  # (M, 51)
    kpts_all = kpts_flat.reshape(M, cfg.num_keypoints, cfg.kpt_step)  # (M, 17, 3)

    candidates: List[PoseDetection] = []
    for i in range(M):
        kpts = kpts_all[i].copy()
        # Keypoint confidence may be raw logit or sigmoid — apply sigmoid to be safe
        kpts[:, 2] = _sigmoid(kpts[:, 2])
    ...
    # NMS is optional (model already did it).  Only run if explicitly requested.
    if cfg.force_nms:
        keep = _nms(candidates, cfg.nms_threshold, cfg.nms_top_k)
        return [candidates[i] for i in keep]
...
def _nms(
    ...
    # If cxcywh -> xyxy for IoU
    # We can't reliably detect which format, so assume already xyxy or cxcywh
    # and handle both: if all values < 2, assume normalized cxcywh
