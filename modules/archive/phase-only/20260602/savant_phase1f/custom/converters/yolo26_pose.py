    ...
    def _call_impl(
        ...
        *output_layers: np.ndarray,
        ...
        if is_detailed and bbox_tensor.shape[0] > 0:
            ...
            if issues:
                print(
                    f"phase=phase1e_converter_bbox_issues issues={'|'.join(issues)}",
                    flush=True,
                )
        ...
        # --- Attrs debug (first _DETAILED_LOG_LIMIT calls only) -----------------
        if attrs_rows and is_detailed:
            ...
            first_attr_tuple_len = len(first_attr_tuple)
            print(
                f"phase=phase1e_converter_attrs_debug "
                f"returned_detections={len(detections)} "
                f"bbox_count={bbox_tensor.shape[0]} "
                f"attrs_type={type(attrs_rows).__name__} "
                f"attrs_len={len(attrs_rows)} "
                f"first_attr_list_type={type(attrs_rows[0]).__name__} "
                f"first_attr_list_len={len(attrs_rows[0])} "
                f"first_attr_tuple_type={type(first_attr_tuple).__name__} "
                f"first_attr_tuple_len={first_attr_tuple_len} "
