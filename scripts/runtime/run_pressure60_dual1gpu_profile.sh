...
Environment overrides:
  ...
  ADAFACE_ALL_INTRA=0   Set to 1 for all-intra 1 FPS central AdaFace input.
...
adaface_all_intra="${ADAFACE_ALL_INTRA:-0}"
...
if [[ "${adaface_all_intra}" == "1" ]]; then
  cmd+=(--adaface-decoupled-all-intra)
fi
...
printf 'adaface_decoupled_all_intra=%s\n' "${adaface_all_intra}"
