-- Migration 004: Phase C1 — Camera / Zone / Rule configuration MVP
--
...
-- cameras). C1 reuses the same pattern as 002_phase2e_events.sql:
-- drop the dead placeholder tables and recreate them per the C1 spec.
--
...
CREATE TABLE camera_rules (
    ...
    created_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
    -- C1-lite: one rule per (camera, rule_type). Future extensions can
