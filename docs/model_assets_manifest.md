...
- TensorRT engine: **built in F2.1** (2026-05-26). Auto-generated by
  Savant nvinfer at `/models/adaface/adaface_ir50_webface4m.onnx_b16_gpu0_fp16.engine`.
  Batch size 16, fp16, T4 GPU.
...
- Phase required: **F2**.
- F2.1 runtime status: **verified** (2026-05-26).
  - Runtime uses `feature` layer only (`norm` layer not configured).
  - `TensorToVectorConverter` output is L2-normalized at runtime
    (raw_l2_norm=1.0 observed), contradicting earlier ONNX inspection
    which showed raw features were NOT L2-normalized. The converter
    applies L2 normalization internally.
  - Feature metadata namespace: `adaface` / `feature`.
  - feature_dim=512 confirmed.
  - person_track_id association preserved through AdaFace stage.
  - landmarks remain readable after AdaFace stage.
