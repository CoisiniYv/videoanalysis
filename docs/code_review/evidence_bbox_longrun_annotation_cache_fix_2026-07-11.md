...
cache. Every retry reused the same cached, incomplete Redis range, so
`latest_frame_pts` never advanced even after the exporter caught up. After the
30-second wait limit, media-worker accepted `missing_frame_metadata` with zero
annotation rows, indexed that empty result, and pruned the successful bundle's
sidecars.
...
- Once the reader proves that `latest_frame_pts` is behind the event anchor,
  every retry must bypass the range cache and read the live per-source Redis
  stream.
