...
exact-lease 修复 `2a57f20`，当前容量结构 checkpoint `fa7731f`，pressure restore
修复 `da35730`。
...
| 缓存 | `rolling-cache-sink` | 每 source/session H.264 passthrough、单 worker/128 outstanding FIFO durable publication、stage/commit 与 queue-residence/service/total attribution、生产 preparation group limit=1、原子 fragment/compact manifest/rename、rename 后有界校验 journal、健康与 drain 指标；日常 metadata layout 为 split；default-off single-inode v2 虽减少一个 file fence，但已因 directory-sync/FIFO/visibility 退化而拒绝 |
