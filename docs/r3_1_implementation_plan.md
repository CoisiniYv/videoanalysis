...
When clip generation is enabled later, `raw_clip.mp4` is the canonical
production evidence video. `annotated_clip.mp4` is optional/on-demand only and
must not be generated by default.
...
7. frontend clients render dynamic overlays from `metadata.json`; production
   must not create default dual video output.
